  ...
  if (!level) {
    return (
      <div className="pt-40 pb-20 text-center max-w-md mx-auto px-4">
        ...
        <p className="text-xs text-[#94A3B8] mb-6">The requested Power BI level does not exist.</p>
        <Link to="/projects" className="stripe-btn-primary px-5 py-2.5 text-white font-display font-bold rounded-full text-xs">
          Back to Projects
        </Link>
      </div>
    );
  ...
  return (
    <PageTransition>
      <div className="pt-28 pb-20">
        ...
        {/* Top Breadcrumb */}
        <div className="border-b border-white/10 bg-[#080B1A] py-4 mb-8">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {/* Level Header */}
          <div className="stripe-card p-6 sm:p-10 mb-12 relative overflow-hidden">
            {justCompleted && (
              <div className="absolute top-0 left-0 right-0 py-2.5 bg-[#00D924] text-black font-bold text-center text-xs flex items-center justify-center gap-2 shadow-lg">
            ...
            <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-8 mt-2">
              <div className="max-w-3xl">
                <div className="flex items-center gap-2.5 flex-wrap mb-4 font-mono text-xs">
                  <span className="font-bold px-3 py-1 rounded-md bg-[#1E293B] text-[#00D4FF] border border-white/10">
                    LEVEL {String(level.id).padStart(2, "0")}
                  </span>
                  <span className="px-3 py-1 rounded-full bg-[#0F172A] border border-white/10 text-white">
                    {level.difficulty}
                  </span>
                  <span className="text-[#94A3B8] flex items-center gap-1">
                    <Clock className="w-3.5 h-3.5 text-[#635BFF]" /> {level.estimatedHours}
                  </span>
                  <span className="text-[#94A3B8] flex items-center gap-1">
                ...
                <p className="text-xs font-mono text-[#00D4FF] font-bold mb-4">
              ...
              <div className="flex flex-col sm:flex-row lg:flex-col gap-3 shrink-0 w-full lg:w-auto font-display">
                <button
                  ...
                  className={`w-full px-6 py-3.5 rounded-full font-bold text-xs transition-all flex items-center justify-center gap-2 ${
                    status === "completed"
                      ? "bg-[#00D924] text-black shadow-lg shadow-[#00D924]/20"
                      : "stripe-btn-primary text-white shadow-lg"
                  }`}
                >
                ...
                <a
                  ...
                  className="w-full px-5 py-3 rounded-full bg-[#1E293B] hover:bg-white/10 text-white font-semibold text-xs border border-white/10 transition-all flex items-center justify-center gap-2"
                >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
            ...
            <div className="lg:col-span-8 space-y-12">
              ...
              {/* Business Scenario */}
              <section className="stripe-card p-6 sm:p-8">
              ...
              <section>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {level.objectives.map((obj, i) => (
                    <div
                      key={i}
                      className="p-4 rounded-xl border border-white/10 bg-[#0F172A] flex items-start gap-3 text-xs text-[#F8FAFC]"
                    >
                      <Check className="w-4 h-4 text-[#00D924] shrink-0 mt-0.5" />
                      <span>{obj}</span>
                    </div>
                  ))}
                </div>
              ...
              {level.businessQuestions.length > 0 && (
                <section className="stripe-card p-6 sm:p-8 border-[#635BFF]/30">
                  ...
                  <ol className="space-y-2.5 list-decimal list-inside text-xs text-[#94A3B8] font-mono">
              ...
              <section>
                ...
                <div className="space-y-4">
                  {level.workflow.map((w, i) => (
                    <div
                      key={i}
                      className="flex items-start gap-4 p-4 rounded-xl border border-white/10 bg-[#0F172A]"
                    >
                      <div className="w-8 h-8 rounded-lg bg-[#1E293B] text-[#00D4FF] font-mono font-extrabold text-xs flex items-center justify-center shrink-0 border border-white/10">
                        {w.step}
                      </div>
                      <div>
              ...
              {/* Deliverables */}
              <section className="stripe-card p-6">
              ...
              <div className="stripe-card p-6 sticky top-24">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="font-display font-bold text-white text-base flex items-center gap-2">
                    <FolderGit2 className="w-4 h-4 text-[#635BFF]" />
              ...
              <div className="stripe-card p-6">
                ...
                <div className="flex flex-wrap gap-1.5 font-mono">
                  {level.skills.map((s, i) => (
                    <span
                      key={i}
                      className="text-xs px-2.5 py-1 rounded-md bg-[#1E293B] border border-white/5 text-[#94A3B8]"
                    >
                      {s}
                    </span>
                  ))}
          ...
          <div className="border-t border-white/10 mt-16 pt-8 flex flex-col sm:flex-row items-center justify-between gap-4">
            {prevLevel ? (
              <Link
                ...
                className="w-full sm:w-auto p-4 rounded-xl border border-white/10 bg-[#0F172A] hover:border-[#635BFF]/50 transition-all flex items-center gap-3"
              >
                <ArrowLeft className="w-5 h-5 text-[#94A3B8]" />
                <div className="text-left">
                  <div className="text-[10px] font-mono text-[#94A3B8] uppercase">Previous Level</div>
            ...
            {nextLevel ? (
              <Link
                ...
                className="w-full sm:w-auto p-4 rounded-xl border border-white/10 bg-[#0F172A] hover:border-[#635BFF]/50 transition-all flex items-center justify-end gap-3 text-right"
              >
                <div>
                  <div className="text-[10px] font-mono text-[#94A3B8] uppercase">Next Level</div>
              ...
              <div className="p-4 rounded-xl bg-[#0F172A] border border-[#635BFF]/40 text-[#635BFF] text-xs font-mono font-bold flex items-center gap-2">
